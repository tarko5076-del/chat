...
class MemoryManager:
    """3-layer memory coordinator: episodic, semantic, and working memory.

    Episodic:   What happened (events, tool traces, outcomes)
    Semantic:   What we learned (user facts, preferences)
    Working:    What's happening now (ConversationMemory)
    """
    ...
    def build_context_string(
        ...
        *,
        ...
        facts = self.get_semantic_facts(db, customer_id=customer_id)
        if not facts:
            return ""
    ...
    def build_snapshot(
        ...
        *,
        ...
        episodic_summary = ""
        if customer_id:
            recent = self.get_episodic_history(db, customer_id=customer_id, limit=5)
            if recent:
        ...
        semantic_facts = ""
        if customer_id:
            semantic_facts = self.build_context_string(db, customer_id=customer_id)

        profile_summary = ""
        if customer_id:
            profile = self.get_profile(db, customer_id=customer_id)
            if profile:
                parts = []
