"""Shared text parsing utilities used across the agent, planner, and tools."""
...
def parse_date(text: str) -> str | None:
    """Parse a date from text. Handles 'tomorrow', 'today', and YYYY-MM-DD."""
    lower = text.lower()
...
def parse_time(text: str) -> str | None:
    """Parse a time from text. Returns HH:MM in 24h format."""
...
def parse_party_size(text: str) -> int | None:
    """Extract party size from text (e.g., 'for 4', '3 people')."""
    match = PARTY_SIZE_RE.search(text)
    return int(match.group(1) or match.group(2)) if match else None


def cuisine_hint_words(text: str) -> set[str]:
    """Get synonym words related to cuisine hints for fuzzy matching."""
    lowered = text.lower()
    hints: set[str] = set()
